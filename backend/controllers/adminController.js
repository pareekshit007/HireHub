...
exports.getStats = asyncHandler(async (req, res) => {
  const [
    ...
    Job.find().sort({ createdAt: -1 }).limit(5)
      .populate({ path: 'employer', select: 'name employerProfile' })  // ← fixed populate
      .select('title status createdAt category employer'),
  ]);
...
exports.getJobs = asyncHandler(async (req, res) => {
  ...
  if (req.query.search) filter.$or = [           // ← fixed: regex instead of $text
  ...
  const [jobs, total] = await Promise.all([
    Job.find(filter)
      .populate({ path: 'employer', select: 'name email employerProfile' })  // ← fixed populate
      .sort({ createdAt: -1 })
      .skip(skip).limit(limit),
...
// ── POST /api/admin/setup — one-time admin bootstrap ─────────────────────────
// Disabled in production. Only works if zero admins exist in the DB.
exports.setupAdmin = asyncHandler(async (req, res) => {
  if (process.env.NODE_ENV === 'production')
    return sendError(res, 'This endpoint is disabled in production.', 403);

  const adminExists = await User.exists({ role: 'admin' });
  if (adminExists)
    return sendError(res, 'An admin already exists. This endpoint is disabled.', 403);

  const { userId, email, setupKey } = req.body;

  if (!setupKey || setupKey !== process.env.ADMIN_SETUP_KEY)
    return sendError(res, 'Invalid setup key', 403);
  ...
  return sendSuccess(res, { email: user.email, role: user.role }, 'User promoted to admin. Please log in again.');
});